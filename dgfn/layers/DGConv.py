...
class MCF_Function(Function):
    @staticmethod
    def forward(ctx, weight, mFilter):
        ctx.save_for_backward(weight, mFilter)
        output = _C.mcf_forward(weight, mFilter)
        return output

    @staticmethod
    def backward(ctx, grad_output):
        weight, mFilter = ctx.saved_tensors
        grad_weight = _C.mcf_backward(grad_output, mFilter)
        return grad_weight, None
...
class MConv(_ConvNd):
    ...
    def expand_bias(self, bias):
        index = []
        bias_size = bias.cpu().detach().numpy().size
...
class DGConv(MConv):
    def __init__(self, in_channels, out_channels, kernel_size, M=4, nScale=3, stride=1,
                    padding=0, dilation=1, groups=1, bias=True, expand=False, padding_mode='zeros', deformable_groups=1):
        super(DGConv, self).__init__(in_channels, out_channels, kernel_size, M, nScale, stride,
                    padding, dilation, groups, bias, expand, padding_mode)
        self.generate_SFilters(self.kernel_size)
        self.generate_GFilters(nScale,self.kernel_size)
        self.MCF_Function = MCF_Function.apply
        self.GOF_Function = GOF_Function.apply
        self.bn = nn.BatchNorm2d(self. out_channels *self.M)
        self.relu = nn.ReLU(inplace=True)
        self.deformable_groups = deformable_groups

    def generate_SFilters(self, kernel_size):
        self.SFilters = Parameter(torch.randn(*kernel_size))

    def generate_GFilters(self, nScale, kernel_size):
        # To generate Gabor Filters
        self.register_buffer('GFilters', getGaborFilterBank(nScale, *kernel_size))
    def forward(self, x, offset):
        if self.expand:
            x = self.do_expanding(x)
        new_weight1 = self.MCF_Function(self.weight, self.SFilters)
        new_bias = self.expand_bias(self.bias) if self.need_bias else self.bias
        mcf_out = deform_conv(x, offset, new_weight1, self.stride, self.padding,
                self.dilation, self.groups, self.deformable_groups)
        mcf_out = self.bn(mcf_out)
        mcf_out = self.relu(mcf_out)
        exp_size = (1,1,) + self.SFilters.size()
        SFilter_exp = self.SFilters.view(exp_size)
        #print(SFilter_exp.size(),'--','self.GFilters.size()')
        new_weight2 = self.GOF_Function(SFilter_exp, self.GFilters)
        for n in range(self.out_channels):
            index = []
            for m in range(self.M):
                index.append(n*self.M+m)
            index = torch.LongTensor(index).cuda()
            if n==0:
                gf_out = F.conv2d(mcf_out.index_select(1,index), new_weight2, None, 1,
                        self.padding, self.dilation, self.groups)
            else:
                gf_out = torch.cat((gf_out, F.conv2d(mcf_out.index_select(1,index), 
                        new_weight2, None, 1, self.padding, self.dilation, self.groups)),1)
        return gf_out        
